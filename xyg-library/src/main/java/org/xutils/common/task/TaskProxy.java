...
/*package*/ class TaskProxy<ResultType> extends AbsTask<ResultType> {
    ...
    /*package*/ TaskProxy(AbsTask<ResultType> task) {
        ...
        // 获取线程池管理类，如果task是HttpTask，调用的是HttpTask的getExecutor方法，应该指向HttpTask中的CACHE_EXECUTOR或HTTP_EXECUTOR
    ...
    protected final ResultType doBackground() throws Throwable {
        ...
        PriorityRunnable runnable = new PriorityRunnable(
                task.getPriority(),// 获取task优先级，这个优先级是在RequestParams中设置的。如果要设置优先级，请在RequestParams设置
                new Runnable() {
                    @Override
                    public void run() {
                        try {
