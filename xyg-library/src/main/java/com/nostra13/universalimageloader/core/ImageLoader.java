...
public class ImageLoader {
	...
	public void displayImage(String uri, ImageAware imageAware, DisplayImageOptions options,
		...
		if (bmp != null && !bmp.isRecycled()) {// 如果缓存中有这个key，那就直接加载，不用请求网络
			L.d(LOG_LOAD_IMAGE_FROM_MEMORY_CACHE, memoryCacheKey);

			if (options.shouldPostProcess()) {// 如果设置了图片处理器，就做下图片处理，然后显示
				ImageLoadingInfo imageLoadingInfo = new ImageLoadingInfo(uri, imageAware, targetSize, memoryCacheKey,
						options, listener, progressListener, engine.getLockForUri(uri));
				ProcessAndDisplayImageTask displayTask = new ProcessAndDisplayImageTask(engine, bmp, imageLoadingInfo,
						defineHandler(options));
				if (options.isSyncLoading()) {// 如果设置了同步加载，就run
					displayTask.run();
				} else {
					engine.submit(displayTask);
				}
			} else {// 如果没有设置图片处理器，就直接显示
				options.getDisplayer().display(bmp, imageAware, LoadedFrom.MEMORY_CACHE);
				listener.onLoadingComplete(uri, imageAware.getWrappedView(), bmp);
			}
		} else {// 缓存中没有该key
			if (options.shouldShowImageOnLoading()) {// 如果设置了图片在下载期间显示的图片，就先加载loading图片
