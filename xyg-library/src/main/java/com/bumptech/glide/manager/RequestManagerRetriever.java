...
public class RequestManagerRetriever implements Handler.Callback {
    ...
    SupportRequestManagerFragment getSupportRequestManagerFragment(final FragmentManager fm) {
        ...
        if (current == null) {
            ...
            if (current == null) {
                // 创建SupportRequestManagerFragment实例，并put到pendingSupportRequestManagerFragments
                // 这里创建了一个Fragment，目的是实现glide跟Activity或Fragment生命周期的绑定
    ...
    RequestManager supportFragmentGet(Context context, FragmentManager fm) {
        ...
        // SupportRequestManagerFragment中RequestManager是通过setRequestManager持有requestManager引用的，
        // 第一次是null
        RequestManager requestManager = current.getRequestManager();
        if (requestManager == null) {
            // 创建RequestManager实例
            requestManager = new RequestManager(context, current.getLifecycle(), current.getRequestManagerTreeNode());
            // 将该应用配置给SupportRequestManagerFragment实例
            current.setRequestManager(requestManager);
        }
        return requestManager;
    }
