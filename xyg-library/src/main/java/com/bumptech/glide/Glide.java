...
public class Glide {
    ...
    public static void setModulesEnabled(boolean enabled) {
        synchronized (Glide.class) {
            // glide不为空回报异常，请在glide初始化前设置modulesEnabled
            if (glide != null) {
        ...
        if (glide == null) {
            synchronized (Glide.class) {
                if (glide == null) {
                    ...
                    // parseGlideModules方法解析<meta-data>元素，并获取GlideModule列表
                    // GlideModule会做一些全局配置工作，这个要在applyOptions方法中通过builder参数来做配置
                    // 可以看下GlideBuilder代码
                    /**
                     ...
                     通过GlideBuilder可以配置内存缓存／磁盘缓存／图片质量等
                     */
    ...
    private static List<GlideModule> parseGlideModules(Context applicationContext) {
        if (modulesEnabled) {
            // 解析Manifest中的<meta-data>元素数据，并获取GlideModule列表
    ...
    Glide(Engine engine, MemoryCache memoryCache, BitmapPool bitmapPool, Context context, DecodeFormat decodeFormat) {
        ...
        /// GenericLoaderFactory：维护一个model到factory到映射
        loaderFactory = new GenericLoaderFactory(context);
        mainHandler = new Handler(Looper.getMainLooper());
        bitmapPreFiller = new BitmapPreFiller(memoryCache, bitmapPool, decodeFormat);

        // 下面有一堆DataLoadProvider的初始化工作，我们先来了解下这些DataLoadProvider是干什么的
        // 首先DataLoadProvider用于Data和Resource之间的编解码，解码表示从 Data 转化为 Resource 的过程，
        // 编码表示将 Data 或者 Resource 持久化为本地文件的过程，以实现磁盘缓存。
